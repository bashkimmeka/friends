...
PassportSchema.statics.updatePassword = function (
  userId: string,
  newPassword: string
) {
...
PassportSchema.statics.getPassword = function (userId: string) {
  return new Promise((resolve, reject) => {
    this.findOne({ userId: userId }).exec(
      (err: string, pwd: IPassportInterface) => {
        if (err) {
...
export const Passport: PassportModel = model<IPassportInterface, PassportModel>(
  "passport",
  PassportSchema
);
